...
strawberry <- read.csv("~/Desktop/MSSP/Fall/MA 615/Strawberry_Activities/strawberry.csv")
...
```{r}
chemicals <- strwb_survey %>% select(State, Domain.Category) %>% filter(Domain.Category!="NOT SPECIFIED")
chemicals <- chemicals %>% filter(!str_detect(chemicals$Domain.Category, "^F"))

chemicals$Domain.Category <- str_replace(chemicals$Domain.Category, "CHEMICAL, ", "")

chemicals <- separate_wider_delim(chemicals, Domain.Category, delim = ": ", names = c("cide", "chemical"))

chemicals$fungicide <- ifelse(chemicals$cide=="FUNGICIDE", 1, 0)
chemicals$herbicide <- ifelse(chemicals$cide=="HERBICIDE", 1, 0)
chemicals$insecticide <- ifelse(chemicals$cide=="INSECTICIDE", 1, 0)
chemicals$other <- ifelse(chemicals$cide=="OTHER", 1, 0)
...
chemicals <- separate_wider_delim(chemicals, chemical, 
                                  delim = " = ", 
                                  names = c("chemical", "pc.code"),
                                  too_few = "align_start")

ext_haz <- toupper(c("Aldicarb", "Brodifacoum", "Bromadiolone", "Bromethalin", "Calcium cyanide", "Captafol", "Chlorethoxyfos", "Chlormephos", "Chlorophacinone", "Difenacoum", "Difethialone", "Diphacinone", "Disulfoton", "EPN", "Ethoprophos", "Flocoumafen", "Hexachlorobenzene", "Mercuric chloride", "Mevinphos", "Oxamyl", "Parathion", "Parathion-methyl", "Phenylmercury acetate", "Phorate", "Phosphamidon", "Sodium fluoroacetate"))



chemicals$toxicity <- ifelse(chemicals$chemical %in% ext_haz, "EXTREMELY HAZAARDOUS", NA)
chemicals$CAScode <- NA
```
